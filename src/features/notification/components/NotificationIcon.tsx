...
const COLOUR_MAP: Record<NotificationType, { bg: string; icon: string }> = {
...
export function NotificationIcon({ type, className }: NotificationIconProps) {
  const { bg, icon } = COLOUR_MAP[type];

  return (
    <div
      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${bg} ${className ?? ''}`}
    >
      <span className={`h-5 w-5 ${icon}`}>{ICON_MAP[type]}</span>
    </div>
  );
}
